 ...
 * Colours
 */
const coloursModule = fs.readFileSync('node_modules/@gocommerce/tachyons/src/_colors.css')

const coloursRoot = postcss.parse(coloursModule)

const groupByAlpha = R.groupBy(R.compose(R.test(/(rgba.+|transparent)/), R.last))
...
fs.writeFile('./src/api/colours.json', JSON.stringify(colours, null, 2), err => {
  if (err) {
