...
import R from 'ramda'
import { allSelectorsToClassNames, nonMediaValuesBy, root, renameKeys } from '../../utils'

const valuesFromModule = (valueGetter, moduleName) => {
  const module = fs.readFileSync(`node_modules/@gocommerce/tachyons/src/_${moduleName}.css`)
...
const scaleParsers = {
  ...
  'font-weight': R.compose(R.omit(['.b', '.normal']), nonMediaValuesBy(R.prop('fontWeight'))),

  'border-radius': nonMediaValuesBy(R.ifElse(R.has('borderRadius'), R.prop('borderRadius'), R.identity)),

  'border-widths': nonMediaValuesBy(R.ifElse(R.has('borderWidth'), R.prop('borderWidth'), R.identity)),
  ...
  'max-widths': R.compose(R.omit(['.mw-none']), nonMediaValuesBy(R.prop('maxWidth'))),
